...
(b)
Let $\bar X$ denote the sampling distribution (n=50). The underlying population distribution is normal, so $\bar X$~$N(\mu, \frac{\sigma^2}{n})$ = $\bar X$~$N(128.0, \frac{10.2^2}{50})$  

The probability of sample mean of blood pressure of 50 males between 20 and 29 yaers old to be less than 125.0 is $P(\bar X<125.0)=$ **`r round(pnorm(125, 128, (10.2^2)/50), 2)`**

(c)
When sample size is 40, the sampling distribution $\bar X$~$N(128.0, \frac{10.2^2}{40})$   
The 90th percentile of this distribution is **`r round(qnorm(0.9, 128, (10.2^2)/40), 2)`**
...
$|t|>t_{n-1, 1-\alpha/2}$ and we fail to reject the null hypothesis at a significance level of .01.

Thus, the mean pulse of young women suffering from fibromyalgia does not differ significantly from 70.  
